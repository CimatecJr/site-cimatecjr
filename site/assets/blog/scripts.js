...
function show_text(el, btn) {
  var text = document.getElementById(el).style.display;
...
function search_bar(el, chb) {
  var input = document.getElementById(el);
 
  var all_check = document.querySelectorAll('[type=checkbox]:checked');
  var values = [];

  for (let index = 0; index < all_check.length; index++) {
    values.push(all_check[index].value);
  }
